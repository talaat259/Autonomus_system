...
class RRT_Path_Plan_node:
  ...
  def __init__(self,namespace='rrt'):
    ...
    rospy.Subscriber("/cam_lidar_fusion_cones",MarkerArray,self.cones_pipeline)
      ...
      for leaf in leafNodes:
          ...
          while node.parent is not None:
              ...
              for cone in largerGroupFrontCones:
                ...
                if cone.color.data=="blue_cone":
                  self.leftCones.append(cone)
                elif cone.color.data == "yellow_cone":
              ...
              if ((len(self.leftCones) == 0 and len(self.rightCones)) > 0 or (len(self.leftCones) > 0 and len(self.rightCones) == 0)):
                  # print "cones are only from one side, penalize rating"
                  nodeRating /= bothSidesImproveFactor
    
              if (len(self.leftCones) > 0 and len(self.rightCones) > 0):
  ...
  def coneIsBlue(self,cone):
    try:
      if(self.leftCones.index(cone)):
        return True
    except ValueError:
        return False   
  ...
  def getDelaunayEdges(self,cones_x,cones_y):
    ...
    for i in range(len(cones_x)):
      cone = Cone()
      cone.position.x = cones_x[i]
      cone.position.y = cones_y[i]
      if(self.coneIsBlue(cone)):
        blueConeCounter+=1
      else:
        yellowConeCounter+=1
      if(blueConeCounter>2 or yellowConeCounter>2):
        return
...
class RRT():
  ...
  def __init__(self, start, planDistance, obstacleList, expandDis=0.5, turnAngle=70, maxIter=1000, rrtTargets = None):
