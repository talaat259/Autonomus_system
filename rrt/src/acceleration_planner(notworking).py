...
class acceleration_planner:
  def __init__(self): #,namespace='rrt'
    ...
    savedPoints = rospy.Subscriber("/cam_lidar_fusion_cones",MarkerArray,self.cones_callback)
  ...
  def cones_callback(self,cones_msg):
    ...
    for cone in cones_msg.markers:
      ...
      if cone.color.g == 255.0 and cone.color.b == 255.0:
        self.right_conesX.append(x)
        self.right_conesY.append(y)
        self.ri+=1
      elif cone.color.b ==255.0:
    ...
    if self.c == 0: 
      if min(self.left_conesX) < min(self.right_conesX):
        self.firstPos = min(self.left_conesX)
      else:
         self.firstPos = min(self.right_conesX)
      self.c+=1

    self.cones_visuals(right_conesX, right_conesY, left_conesX, left_conesY)
    self.acceleration_path_algorithm(right_conesX, right_conesY, left_conesX, left_conesY)
  ...
  def acceleration_path_algorithm(self,right_conesX ,right_conesY,left_conesX ,left_conesY):
        ...
        points = numpy.vstack((xcoeff,ycoeff)).T
        # print(points)
        ...
        self.publishWaypointsVisuals(points)
  ...
  def publishWaypointsVisuals(self, points = None):
      ...
      savedWaypointsMarker.header.frame_id = "odom"
