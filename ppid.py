...
class stanley_controller:
  ...
  def control_callback(self,control_msg):
    ...
    self.control_elf.Vx = control_msg.drive.speed
  ...
  def stanley_control(self):
    ...
    self.Ld=math.sqrt((self.x+self.L)**2 + (self.y)**2)
    self.look_ahead.append(self.Ld)
    self.alpha=math.atan(self.y/(self.x+self.L))
    self.eta=math.atan((2*self.L*math.sin(self.alpha))/self.Ld)
    
    
    time = rospy.get_rostime()    ####
    self.now.append(time)     ####
    self.eta_plot.append(self.eta)     ####
  ...
  def control_command(self,steering,velocity):
    ...
    if steering >1.1 or steering <-1.1 or velocity<2.0:
      velocity=2.0
	    
    self.velocity_now.append(velocity)
